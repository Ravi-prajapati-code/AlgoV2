...
from config.settings import IGNORE_SYMBOLS, BLOCKED_SECTORS, BLOCKED_SYMBOLS, SAFE_HAVEN_SYMBOL, SAFE_HAVEN_ENABLED, GOLDBEES_PROFIT_EXIT_ONLY, GOLDBEES_MAX_LOSS_PCT, ENTRY_MODE, ENTRY_MODE_SEED, SECTOR_DURABILITY_WEIGHT, SECTOR_RS_WEIGHT, EXIT_TREND_EMA, EXIT_TREND_CONFIRM_DAYS
...
def generate_signals(
    ...
    for symbol, check_ind, ind, gate_reason in prelim:
        ...
        if SECTOR_RS_WEIGHT and sector_rs:
            score += SECTOR_RS_WEIGHT * sector_rs.get(get_sector(symbol), 0.0)
        candidates.append({
            "symbol": symbol,
            "rs_rank": score,
            "reason": gate_reason,
