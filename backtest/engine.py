...
def _compute_sector_durability(trades: List[Trade], as_of: date) -> Dict[str, float]:
    ...
    for t in trades:
        ...
        if not (window_start <= exit_d < as_of):
            continue
            ...
            for exec_time_str in EXECUTION_TIMES:
                ...
                if hybrid_mode == "defensive":
                    ...
                    for pos in [p for p in open_positions if p.symbol != GOLD_ETF]:
                        ...
                        bear_swing_exit_dates[pos.symbol] = today_date
                    ...
                    if bear_slots_free > 0 and cash > portfolio_val * 0.005:
                        ...
                        for symbol, ind in indicators.items():
                            if symbol in held_symbols or symbol == GOLD_ETF:
                                continue
                            if symbol in bear_swing_exit_dates:
                                days_since = (today_date - bear_swing_exit_dates[symbol]).days
                                if days_since < BEAR_SWING_COOLDOWN_DAYS:
                                    continue
                    ...
                    if num_to_buy > 0:
                        ...
                        for pos in open_positions:
                            if pos.symbol == SAFE_HAVEN_SYMBOL:
                                continue  # never pyramid safe haven (matches live manager)
                        ...
                        if best_pos and best_pos.symbol in prices:
                            raw_add_price = prices[best_pos.symbol]
                            ind_best = indicators.get(best_pos.symbol, {})
                            ema20_add = ind_best.get("ema_20") or ind_best.get("ema_fast") or 0
                            if ema20_add > 0 and (raw_add_price - ema20_add) / ema20_add > 0.05:
                                logger.info(
                                    "  [SKIP_ADD] %s extended >5%% above EMA20 — waiting for pullback",
                                    best_pos.symbol,
                                )
                                best_pos = None
                        if best_pos and best_pos.symbol in prices:
                            ...
                            max_pos_value = portfolio_val * MAX_STOCK_ALLOCATION_PCT
